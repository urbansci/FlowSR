# This script is used to generate data and run symbolic regression for Guangdong mobility data at the county level.
...
if build_x_y_from_raw
    # Load the data
    flow_array = npzread(raw"/path/to/gd_mobility_flow_matrix_intercounty.npy")
    ...
    oppo_array = npzread(raw"/path/to/gd_mobility_opportunity_matrix_intercounty_pop.npy")
    ...
    function min_max_normalize(data)
        norm_data = (data .- minimum(data)) ./ (maximum(data) - minimum(data))
        return norm_data
    end
    ...
    # Prepare x(input) and y(groundtruth) data
    y =  flow
    X = (
        Dist=dist, 
        Oppor = oppo,
    ...
    if save_x_y
        save("gd_mobility_county_X_dist_odwp.jld2", "X", X)
        save("gd_mobility_county_Y.jld2", "y", y)
        save("gd_mobility_county_sep.jld2", "sep", ori_sep)
    end
else
    # Load the x and y data
    X = load("gd_mobility_county_X_dist_odwp.jld2", "X")
    y = load("gd_mobility_county_Y.jld2", "y")
    ori_sep = load("gd_mobility_county_sep.jld2", "sep")
end
